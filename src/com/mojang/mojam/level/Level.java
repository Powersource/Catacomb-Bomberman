...
/*
 * for (int i = 0; i < 10; i++) { double x = random.nextInt(width) * Tile.WIDTH
 * + Tile.WIDTH / 2; double y = random.nextInt(height) * Tile.HEIGHT +
 * Tile.HEIGHT / 2; addEntity(new SpawnerEntity(x, y,
 * random.nextInt(Team.MaxTeams))); }
 */
    }

    public static Level fromFile(String path) throws IOException {
        BufferedImage bufferedImage = ImageIO.read(MojamComponent.class.getResource(path));
    ...
    public void init() {
        ...
        for (int i = 0; i < 11; i++) {
            double x = (random.nextInt(width - 16) + 8) * Tile.WIDTH + Tile.WIDTH / 2;
        ...
        addEntity(new ShopItem(32 * (width / 2 - 1.5), 4.5 * 32, ShopItem.SHOP_TURRET, Team.Team2));
        addEntity(new ShopItem(32 * (width / 2 - .5), 4.5 * 32, ShopItem.SHOP_HARVESTER, Team.Team2));
        addEntity(new ShopItem(32 * (width / 2 + .5), 4.5 * 32, ShopItem.SHOP_BOMB, Team.Team2));
...
//        addEntity(new Bomb(1024 - 80, 360));
    ...
    public void setTile(int x, int y, Tile tile) {
        ...
        for (int of : neighbourOffsets) {
            ...
            if (nbIndex >= 0 && nbIndex < width * height) {
                ...
                if (neighbour != null) neighbour.neighbourChanged(tile);
            }
        }
    }
    ...
    public Set<Entity> getEntities(double xx0, double yy0, double xx1, double yy1, Class<? extends Entity> c) {
        int x0 = (int) (xx0) / Tile.WIDTH;
        int x1 = (int) (xx1) / Tile.WIDTH;
        int y0 = (int) (yy0) / Tile.HEIGHT;
        int y1 = (int) (yy1) / Tile.HEIGHT;
    ...
    public void tick() {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            if (tile instanceof FloorTile) {
                double r = 32 * 8;
                if (getEntities(new BB(null, x - r, y - r, x + r, y + r), Player.class).size() == 0) {
                    r = 32 * 8;
    ...
    public void render(Screen screen, int xScroll, int yScroll) {
        ...
        Set<Entity> visibleEntities = getEntities(xScroll - Tile.WIDTH, yScroll - Tile.HEIGHT, xScroll + screen.w + Tile.WIDTH, yScroll + screen.h + Tile.HEIGHT);
        // Sort the entities
/*
 * final int EntityBlitPixelRowOffset = -12; Set<Entity> visibleEntities =
 * getEntities(xScroll - Tile.WIDTH, yScroll - Tile.HEIGHT, xScroll + screen.w +
 * Tile.WIDTH, yScroll + screen.h + Tile.HEIGHT); ArrayList<Entity>[]
 * rowEntities = new ArrayList[8 + y1 - y0]; // fuck // me... // And how to
 * remove this warning (without supressing)? ^ for (Entity e : visibleEntities)
 * { int y = (int) ((e.pos.y - EntityBlitPixelRowOffset) / Tile.WIDTH) - y0 + 2;
 * if (rowEntities[y] == null) rowEntities[y] = new ArrayList<Entity>();
 * rowEntities[y].add(e); }
 */
...
/*
 * for (int y = y0; y <= y1 + 2; y++) { for (int x = x0; x <= x1; x++) { if (x <
 * 0 || x >= width || y < 0 || y >= height) { screen.blit(Art.floorTiles[5][0],
 * x * Tile.WIDTH, y * Tile.HEIGHT); continue; } int xt = x - 28; int yt = y -
 * 4; if (xt >= 0 && yt >= 0 && xt < 7 && yt < 4) {
 * screen.blit(Art.startLordLard[xt][yt], x * Tile.WIDTH, y * Tile.HEIGHT);
 * continue; } yt = y - (64 - 8); if (xt >= 0 && yt >= 0 && xt < 7 && yt < 4) {
 * screen.blit(Art.startLordLard[xt][yt], x * Tile.WIDTH, y * Tile.HEIGHT);
 * continue; } if (canSee(x, y)) { tiles[x + y * width].render(screen); } } //
 ...
 */
        ...
        for (int y = y0; y <= y1; y++) {
            ...
            for (int x = x0; x <= x1; x++) {
                ...
                if (count == 4) {
                    screen.blit(Art.darkness[1][1], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                } else if (count == 3) {
                    if (!c0) screen.blit(Art.darkness[1][4], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (!c1) screen.blit(Art.darkness[0][4], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (!c2) screen.blit(Art.darkness[1][3], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (!c3) screen.blit(Art.darkness[0][3], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                } else if (count == 1) {
                    if (c0) screen.blit(Art.darkness[2][2], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (c1) screen.blit(Art.darkness[0][2], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (c2) screen.blit(Art.darkness[2][0], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (c3) screen.blit(Art.darkness[0][0], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                } else {
                    if (c0 && c3) screen.blit(Art.darkness[2][4], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (c1 && c2) screen.blit(Art.darkness[2][3], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (c0 && c1) screen.blit(Art.darkness[1][2], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (c2 && c3) screen.blit(Art.darkness[1][0], x * Tile.WIDTH, y * Tile.WIDTH + yo);
                    if (c0 && c2) screen.blit(Art.darkness[2][1], x * Tile.WIDTH, y * Tile.WIDTH + yo);
            ...
            if (!e.removed) {
                if (e.minimapIcon >= 0) {
                    ...
                    if (x >= 0 && y >= 0 && x < width && y < height) {
                        if (hasSeen(x, y)) {
                            minimap.blit(Art.mapIcons[e.minimapIcon % 4][e.minimapIcon / 4], x - 2, y - 2);
                        }
                    }
                }
            }
...
/*
 * int x0 = x - radius; int x1 = x + radius; int y0 = y - radius; int y1 = y +
 * radius; double radiusSqr = radius * radius; if (x0 < 0) x0 = 0; if (y0 < 0)
 * y0 = 0; if (x1 > width) x1 = width; if (y1 > height) y1 = height; for (int yy
 * = y0; yy <= y1; yy++) { double yd = yy - (y + 0.5); for (int xx = x0; xx <=
 * x1; xx++) { double xd = xx - (x + 0.5); double dist = xd * xd + yd * yd; if
 * (dist < radiusSqr) seen[xx + yy * (width + 1)] = true; } }
 */
    }
    ...
    private void revealLine(int x0, int y0, int x1, int y1, int radius) {
        for (int i = 0; i <= radius; i++) {
            ...
            int xd = xx - x0;
            int yd = yy - y0;
            if (xd * xd + yd * yd > radius * radius) return;
            Tile tile = getTile(xx, yy);
            if (tile instanceof WallTile) return;
    ...
    public void placeTile(int x, int y, Tile tile, Player player) {
        if (!getTile(x, y).isBuildable()) return;
